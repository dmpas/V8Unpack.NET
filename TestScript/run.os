...
Функция Упаковать(Знач Каталог, Знач Расширение)

	Лог.Информация("Упаковка средствами утилиты %1", Каталог);

	Файл = ВременныеФайлы.НовоеИмяФайла(Расширение);
...
Функция ВыгрузитьФайлыОбработки(Знач ПутьКФайлу)
	...
	Конфигуратор = Новый УправлениеКонфигуратором();
	Конфигуратор.ВыгрузитьВнешниеОтчетыИлиОбработкиВФайлы(ВременныйКаталог, ПутьКФайлу);
...
Процедура ПротестироватьКонфигурацию(Знач ПутьКФайлу)
	...
	СобранныйФайл = Упаковать(КаталогВыгрузкиУтилиты, "cf");
...
Процедура ПротестироватьВнешнююОбработку(Знач ПутьКФайлу)
	...
	КаталогВыгрузкиУтилиты = Распаковать(ПутьКФайлу);
	ВыгрузкаИсходнойОбработки = ВыгрузитьФайлыОбработки(ПутьКФайлу);
	СобранныйФайл = Упаковать(КаталогВыгрузкиУтилиты, Файл.Расширение);
	ВыгрузкаСобраннойОбработки = ВыгрузитьФайлыОбработки(СобранныйФайл);

	СравнитьКаталоги(ВыгрузкаИсходнойОбработки, ВыгрузкаСобраннойОбработки);

	ВременныеФайлы.УдалитьНакопленныеВременныеФайлы(ФайлыДоТеста);

КонецПроцедуры

Процедура ПроверитьОбработки()

	ТестовыеКонфигурации = НайтиФайлы("epf", "*.epf");
	Для каждого ТестоваяКонфигурация Из ТестовыеКонфигурации Цикл
		Лог.Информация("Обрабатывается внешняя обраобтка %1", ТестоваяКонфигурация.ПолноеИмя);
		ПротестироватьВнешнююОбработку(ТестоваяКонфигурация.ПолноеИмя);
	КонецЦикла;

	ТестовыеКонфигурации = НайтиФайлы("epf", "*.erf");
	Для каждого ТестоваяКонфигурация Из ТестовыеКонфигурации Цикл
		Лог.Информация("Обрабатывается внешняя обраобтка %1", ТестоваяКонфигурация.ПолноеИмя);
		ПротестироватьВнешнююОбработку(ТестоваяКонфигурация.ПолноеИмя);
	КонецЦикла;
...
Процедура ПроверитьКонфигурации()
